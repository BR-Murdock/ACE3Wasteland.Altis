...
_setupObjects =
{
	...
	_vehDeterminer = if ("AEIMO" find (_vehicleName select [0,1]) != -1) then { "A" } else { "An" };
...
_successExec =
{
	...
	_successHintMessage = format ["Congratulations! %1 changed the owner - without one penny!", _vehicleName];
};
