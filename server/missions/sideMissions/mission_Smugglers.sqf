...
_setupObjects =
{
	...
	_smugglerVeh = ["I_G_Offroad_01_armed_F","O_G_Offroad_01_armed_F","B_G_Offroad_01_armed_F"] call BIS_fnc_selectRandom; 
	...
	[_aiGroup,_missionPos] spawn createGroup5;
	...
	_missionHintText = format ["Smugglers try to sell weapons. Show them the real price for these weapons!", sideMissionColor];
};
...
_successExec =
{
	...
	_explosivePos = getPosATL (_smugglerVeh select 0);
	_explosive = createMine ["SatchelCharge_F", _explosivePos, [], 0];
	_explosive setDamage 1;
	
	_successHintMessage = format ["Good work! The weapons has been sold - nearly for free."];
};
